...
class AIReaderApp {
    ...
    async handleSearch() {
        ...
        const query = document.getElementById('textInput')?.value.trim() || document.getElementById('urlInput')?.value.trim() || '';

        if (!query && !(window.uploadManager && window.uploadManager.hasUploadedContent())) {
        ...
        if (!this.currentUser) {
            ...
            if(window.authManager) window.authManager.showAuthModal('login');
            return;
        }
        ...
        try {
            ...
            switch (this.currentFunction) {
                ...
                case 'fact-checking':
                    if (window.progressManager) {
                        ...
                        const endpoint = this.getAnalysisEndpoint(); // 用你的方法获取API地址
                        ...
                        .then(res => {
                            if (!res.ok) return res.json().then(err => { throw new Error(err.message || `服务器错误: ${res.status}`) });
                            return res.json();
                        })
                        .then(result => {
                            hideLoading();
                            if (result.success && result.analysis && Array.isArray(result.analysis.steps)) {
                                window.progressManager.showStepsFromJson(result.analysis.steps);
                            } else {
                                showNotification(result.message || '分析失败，未返回有效步骤', 'error');
                            }
                        })
                        .catch(err => {
                            hideLoading();
                            console.error('分析请求失败:', err);
                            showNotification(err.message || '分析请求网络错误', 'error');
                        });
                    }
                    break;
            ...
            // 清空输入
            if(window.uploadManager) window.uploadManager.clearUploads();
    ...
    async prepareRequestData(query) {
        ...
        /**
     * 新增：构建 FormData 用于文件/图片上传
     * @param {object} data - 包含文本等信息的数据对象
     * @returns {FormData}
     */
    buildFormData(data) {
        const formData = new FormData();
        const activeUpload = window.uploadManager ? window.uploadManager.getInputContent() : { type: 'text', data: data.text };

        formData.append('content_type', activeUpload.type);

        switch (activeUpload.type) {
            case 'text':
            case 'url':
                formData.append('content_data', activeUpload.data || '');
                break;
            case 'file':
                if (activeUpload.files && activeUpload.files.length > 0) {
                    activeUpload.files.forEach(file => formData.append('files', file));
                }
                break;
            case 'image':
                if (activeUpload.files && activeUpload.files.length > 0) {
                    activeUpload.files.forEach(image => formData.append('images', image));
                }
                break;
        }
        return formData;
