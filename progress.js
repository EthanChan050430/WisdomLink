/**
 * ===================================================================
 * 滚动锁定模块 (Scroll Lock Module)
 * 通过 IIFE 模式封装，避免全局污染，代码结构更清晰。
 * ===================================================================
 */
const scrollLock = (function() {
    // --- 模块私有变量 ---
    let scrollY = 0;
    const lockedClassName = 'body-scroll-locked';
    ...
    return {
         ...
         */
        lock: function() {
            if (document.body.classList.contains(lockedClassName)) {
                return;
            }
...
class ProgressManager {
    constructor() {
        ...
        this.stepScrollPositions = {}; // <-- 用于记录每个步骤的滚动位置
        this.activeModalStepId = null;   // <-- 记录当前打开的是哪个模态框
        this.setupMarkdown();
        this.bindEvents();
        console.log('[进度管理器] 初始化完成，滚动位置记录本已创建:', this.stepScrollPositions);
    }
     ...
     */
    closeModal() {
        if (this.activeModalStepId) {
            const contentContainer = document.getElementById('stepDetailContent');
            const currentScroll = contentContainer.scrollTop;
            this.stepScrollPositions[this.activeModalStepId] = currentScroll;
            console.log(`[关闭时] 正在为步骤 ${this.activeModalStepId} 保存滚动位置:`, currentScroll);
            console.log('[账本状态]', this.stepScrollPositions);
            this.activeModalStepId = null; 
        }
    ...
    /**
     * 用于JSON批量输出时，依次展示每个步骤内容（不影响原有流式功能）
     * @param {Array} stepsArr - JSON中的steps数组
     */
    showStepsFromJson(stepsArr) {
        this.currentSteps = stepsArr.map(({step, name, description, icon}) => ({
            id: step,
            title: name,
        ...
        const unlockNext = () => {
            if (i < stepsArr.length) {
                this.updateStep(stepsArr[i].step, stepsArr[i].content, 'content');
                this.updateStep(stepsArr[i].step, '', 'complete');
                i++;
                setTimeout(unlockNext, 600); // 600ms后显示下一个
            }
        };
        unlockNext();
    }
    ...
    showStepDetail(stepId) {
        ...
        // 【核心】恢复此步骤之前保存的滚动位置
        setTimeout(() => {
            const savedScrollTop = this.stepScrollPositions[stepId] || 0;
            console.log(`[打开时] 正在为步骤 ${stepId} 恢复滚动位置:`, savedScrollTop);
